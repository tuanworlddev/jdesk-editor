...
export function SourceControlPanel() {
  ...
  if (!status?.available) {
    return (
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-2 px-3 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[--color-fg-dim]">
          <Icon path={mdiSourceBranch} size={15} /> Source Control
        </div>
        <div className="px-4 py-3 text-[12px] text-[--color-fg-dim]">Not a Git repository.</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center gap-2 px-3 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[--color-fg-dim]">
        <Icon path={mdiSourceBranch} size={15} /> Source Control
        <button title="Refresh" onClick={refresh} className="ml-auto text-[--color-fg-dim] hover:text-[--color-fg]">
          <Icon path={mdiRefresh} size={15} />
        </button>
      </div>
      <div className="flex items-center gap-2 px-3 pb-2 text-[12px]">
        <Icon path={mdiSourceBranch} size={14} className="text-[--color-green]" />
        <span className="text-[--color-fg]">{status.branch || 'detached'}</span>
        {(status.ahead > 0 || status.behind > 0) && (
          <span className="text-[--color-fg-dim]">↑{status.ahead} ↓{status.behind}</span>
        )}
        <button title="Commit (via terminal in v1)" disabled
          className="ml-auto flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] text-[--color-fg-dim] opacity-50">
          <Icon path={mdiSourceCommit} size={14} /> Commit
        </button>
      </div>
      <div className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-[--color-fg-dim]">
        Changes {status.files.length > 0 && <span className="rounded bg-[--color-selection] px-1.5">{status.files.length}</span>}
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {status.files.length === 0 && <div className="px-4 py-2 text-[12px] text-[--color-fg-dim]">No changes.</div>}
        {status.files.map((f) => {
          const b = badge(f);
          ...
          return (
            <div key={f.relPath}
              className="group flex h-[24px] cursor-pointer items-center gap-2 px-3 hover:bg-[--color-selection]"
              ...
              {dir && <span className="truncate text-[11px] text-[--color-fg-dim]">{dir}</span>}
